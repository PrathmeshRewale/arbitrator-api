...
public interface MediationFormDocumentsRepository extends JpaRepository<MediationFormDocuments, Long> {
    @Query(value = "SELECT * FROM mediation_form_documents a WHERE a.admission_form_id = :id", nativeQuery = true)
    MediationFormDocuments findByMediationFormId(long id);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM mediation_form_documents a WHERE a.admission_form_id = :id", nativeQuery = true)
